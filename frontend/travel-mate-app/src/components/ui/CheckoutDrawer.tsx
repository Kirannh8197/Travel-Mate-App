//V's_new_start — Premium-Plus: Liquid-Glass Checkout Drawer
// Replaces the old /payment route. Full 3-step flow: Payment → Cab → Done.
// Slides up as an immersive overlay, keeping the user in the hotel context.

import { useState, useEffect, useRef } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import {
    X, CreditCard, Lock, Check, Car, Star, Sparkles, ArrowRight,
...
// ── UPI SVG Icons (preserved from original PaymentPage) ────────────────
const GPayIcon = () => (
            ...
            <AnimatePresence mode="wait">
                {mode === 'card' && (
                    <motion.div key="card" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                        ...
                        <button onClick={handleCardPay} disabled={processing || done}
                            className="w-full mt-5 py-3.5 rounded-2xl font-black text-white text-base shadow-2xl shadow-[var(--tm-ethereal-purple)]/30 transition-all active:scale-95 disabled:cursor-not-allowed"
                ...
                {mode === 'upi' && (
                    <motion.div key="upi" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                        ...
                        {/* V's_new_start — smart UPI input: 10-digit cap for phone-based, free-form for @ID */}
                        <div className="mb-4">
                            <div className="relative">
                                <input
                                    value={upiId}
                                    onChange={e => {
                                        if (upiApp !== 'other') {
                                            // Phone-based UPI (GPay/PhonePe) — digits only, max 10
                                            const digits = e.target.value.replace(/\D/g, '').slice(0, 10);
                                            setUpiId(digits);
                                        } else {
                                            // UPI ID format (name@bank) — free text, max 50 chars
                            ...
                            {upiApp !== 'other' && upiId.length > 0 && upiId.length < 10 && (
                                <p className="text-xs text-amber-600 font-bold mt-1.5 ml-1">
                                    UPI mobile number must be exactly 10 digits
                                </p>
                            )}
                        </div>
                        {/* V's_new_end */}
                        ...
                        <button onClick={handleUpiSend} disabled={!upiId || processing || done || upiSent}
                            ...
                            style={{ background: 'linear-gradient(135deg, var(--tm-deep-indigo), var(--tm-ethereal-purple))' }}>
                            {done ? <><Check className="w-5 h-5" />Confirmed!</> : upiSent ? <><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />Confirming…</> : <><Smartphone className="w-4 h-4" />Pay ₹{booking.grand?.toLocaleString('en-IN')} via UPI</>}
                        </button>
                    </motion.div>
                )}
            </AnimatePresence>
...
export const CheckoutDrawer = ({ isOpen, onClose, booking }: CheckoutDrawerProps) => {
    ...
    // Reset step when drawer is reopened
    useEffect(() => {
        if (isOpen) setStep(0);
    }, [isOpen]);
                ...
                <>
                    ...
                    >
                        ...
                        <div className="px-6 py-5 max-w-lg mx-auto">
                            ...
                            <AnimatePresence mode="wait">
                                {step === 0 && (
                                    <PaymentStep key="pay" booking={booking} onPaid={() => setStep(1)} />
                                )}
                                {step === 1 && (
                                    <ArrivalStep key="arrive" booking={booking}
                                        onSkip={() => { setHasRide(false); setStep(2); }}
                                        onRideBooked={() => { setHasRide(true); setStep(2); }}
                                    />
                                )}
                                {step === 2 && (
                                    <ConfirmStep key="done" booking={booking} hasRide={hasRide} onClose={onClose} />
                                )}
                            </AnimatePresence>
                        </div>
                    </motion.div>
