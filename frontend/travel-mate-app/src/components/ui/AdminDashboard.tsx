//V's_new_start — Sovereign Admin: God-Mode Expansion
// Tabs: [Pending Approvals] [All Hotels] [User Management]
// All Hotels: shows every hotel regardless of status + De-list / Approve actions
// Deep Inspect: click any hotel → side drawer with full hotel details
// User Management: lists all registered users with their roles
...
import {
    ...
    Users, LayoutList, ChevronRight, X, MapPin, Star, DollarSign,
    Mail, Hash, Wifi, RefreshCw
} from 'lucide-react';
...
export const AdminDashboard = () => {
    ...
    const fetchHotels = async () => {
        setIsLoading(true);
        try {
            const res = await fetch(`${BACKEND}/api/hotel`, {
                headers: { 'userid': user?.userId?.toString() || '' }
            });
            if (res.ok) { const d = await res.json(); setHotels(d.data || []); }
    ...
    const otherHotels    = hotels.filter(h => h.status !== 'PENDING_VERIFICATION' && h.status !== 'APPROVED');

    return (
        <>
        {/* V's_new_start — Sovereign Admin: God-Mode Tabbed Hub */}
        <div className="elite-container min-h-screen">
            ...
            <div className="mb-10 flex items-start justify-between gap-6">
                ...
                <div className="flex items-center gap-3 mt-2 shrink-0">
                    <button onClick={fetchHotels} className="w-10 h-10 rounded-xl bg-gray-100 hover:bg-gray-200 border border-gray-200 flex items-center justify-center transition-all" title="Refresh data">
            ...
            {/* ── Tab Bar ──────────────────────────────────── */}
            <div className="flex gap-1 p-1.5 bg-gray-100 rounded-2xl mb-8 w-fit">
                {TABS.map(tab => (
                                        ...
                                        <motion.div key={h.hotelId} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                                            ...
                                            <div className="flex justify-between items-start mb-3">
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <p className="text-sm text-gray-500 font-medium">{h.address?.city}, {h.address?.country}</p>
                                                    <p className="text-xs text-gray-400 font-mono mt-1">{h.email}</p>
                                                </div>
                                                <span className="text-xs px-2.5 py-1 rounded-lg bg-amber-50 text-amber-700 font-black border border-amber-200 ml-3 flex-shrink-0">PENDING</span>
                                            </div>
                                            <div className="flex gap-2">
                                                ...
                                                <button onClick={() => handleAction(h.hotelId, 'REJECTED')}
                                                    className="flex-1 py-2.5 rounded-xl bg-red-50 text-red-600 hover:bg-red-500 hover:text-white border border-red-100 font-black text-sm flex items-center justify-center gap-1.5 transition-all">
                                                    <XCircle className="w-4 h-4" /> Reject
                                                </button>
                        ...
                        <motion.div key="users" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                            {users.length === 0 ? (
                                <div className="elite-card p-10 text-center">
                                    ...
                                    <p className="text-xs text-gray-400 font-medium mt-1">Ensure <code className="bg-gray-100 px-1 rounded">GET /api/auth/users</code> is implemented and requires admin auth.</p>
                                    <button onClick={fetchUsers} className="mt-4 px-5 py-2.5 rounded-xl bg-[var(--tm-ethereal-purple)] text-white font-bold text-sm flex items-center gap-2 mx-auto hover:opacity-90 transition">
                            ...
                            <p className="text-xs text-gray-400 font-medium mt-3 text-center">
                                User management read-only. User deletion / role changes require backend endpoint setup.
                            </p>
                        </motion.div>
                    )}
