...
import { apiClient } from '../api/client'; // ── NEW: Secure Axios Client
                ...
                <div className="relative h-44 bg-gradient-to-br from-[var(--tm-deep-indigo)] to-[var(--tm-ethereal-purple)]">
                    <div className="absolute inset-0 flex flex-col justify-end p-6">
                        ...
                        <div className="flex items-center gap-1 mt-1">
                            ...
                            <span className="text-white/70 text-sm font-medium">{hotel?.address?.city || 'City'}, {hotel?.address?.country || 'Country'}</span>
                        </div>
                    </div>
...
export const SandboxBooking = () => {
    ...
    const { data, isLoading } = useQuery({
        queryKey: ['my-bookings', user?._id || user?.userId],
        enabled: !!(user?._id || user?.userId),
        queryFn: async () => {
            const targetId = user?._id || user?.userId;
            const res = await apiClient.get(`/bookings/user/${targetId}`);
            return res.data;
        },
    });

    // ── SECURE AXIOS MUTATION ──
            ...
            <div>
                ...
                {isLoading ? (
                    ...
                    <GlassCard className="p-12 text-center border-gray-100 bg-white shadow-xl">
                        ...
                        <Link to="/search" className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl font-bold text-white text-sm shadow-xl transition-all"
                            style={{ background: 'linear-gradient(135deg, var(--tm-deep-indigo), var(--tm-ethereal-purple))' }}>
                                                    ...
                                                    <div className="flex items-start gap-4">
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            ...
                                                            <div className="flex items-center gap-1 mt-1">
                                                                ...
                                                                <span className="text-xs text-gray-500 font-medium">{hotel?.address?.city || 'City'}, {hotel?.address?.country || 'Country'}</span>
                                                            </div>
