...
// GET - Hotels near a location (e.g., /api/hotels/nearby?lng=77.5946&lat=12.9716&distance=5000)
router.get("/nearby", async (req: Request, res: Response) => {
    try {
        const lng = parseFloat(req.query.lng as string);
        const lat = parseFloat(req.query.lat as string);
        const distance = req.query.distance ? parseInt(req.query.distance as string) : 10000; // Default 10km
...
router.post("/", async (req: Request, res: Response) => {
    try {
        ...
        res.status(201).json({ message: "Hotel created successfully", data: createdHotel });
    } catch (err: any) {
        if (err.code === 11000) {
             return res.status(400).json({ message: "A hotel with this hotelId already exists." });
        }
